...
directory_name=os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..',  '..', 'data', 'kia'))
...
url='http://www.kia.com/us/en/data/dealers/locator/'
...
combined_name=os.path.join(os.path.dirname(directory_name), 'kia.json')
...
for file_name in file_names:
    with open(file_name, 'r') as fd:
        ...
        if 'dealers' in results:
            method = results['dealers']
            if int(method['statusCode']) == 200:
                if 'result' in method:
                    for dealer in method['result']:
                        if 'code' in dealer:
                            dealers[dealer['code']] = dealer
